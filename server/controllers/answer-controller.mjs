...
const gradeAnswer = async (answer) => {
  ...
  if (["DYNAMIC-MEMORY", "DYNAMIC-PATTERN"].includes(q.questionType)) {
    ...
    grade = grade + parseFloat(q.textGrade);
  }
...
const createStudentAnswer = async (req, res, next) => {
  ...
  //Check if its an admin previewing tests
  if (req.permissions.includes("admin")) {
    res.status(201).json({ message: "Test previewed" });
  } else {
    ...
    //Resorting to test order for answers
    answers = test.questions.map((q) => answers.filter((a) => a.qId === q)[0]);

    const totalGrade = answers.reduce((tot, a) => {
      return tot + (a.grade == null ? 0 : a.grade);
    }, 0);

    const totalPossibleGrade = answers.reduce((tot, a) => {
      return tot + (a.possibleGrade == null ? 0 : a.possibleGrade);
    }, 0);
...
const getStudentAnswerByCodeName = async (req, res, next) => {
  ...
  let answersOut;
  try {
    answersOut = await Promise.all(
      studentAnswer.answers.map(async (sa) => {
    ...
    return next(
      new APIError("Could not find questions for student answer", 404)
    );
  }
